...
function getSseClientPromise(dbName) {
  ...
  if (!getMongoUri()) {
    sseClientPromise = Promise.reject(new Error('Invalid/Missing environment variable: "MONGODB_URI"'));
  ...
  const connectPromise = sseClient.connect().then((client) => client.db(dbName || process.env.MONGODB_DB));
  if (process.env.NODE_ENV === "development") {
